...
export function LedIndicator({ active, color, label, onClick, interactive }: LedProps) {
  ...
  const colors = {
    ...
    purple: active ? "bg-fuchsia-500 shadow-[0_0_8px_2px_rgba(217,70,239,0.6)]" : "bg-fuchsia-900/40 border-fuchsia-900",
  };
  ...
  return (
    ...
    >
      ...
      {label && (
        <span className={`text-[10px] font-bold select-none ${active ? 'text-slate-200' : 'text-slate-600'}`}>
          {label}
        </span>
      )}
    </div>
